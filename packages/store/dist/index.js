...
__export(index_exports, {
  ...
  useDataPart: () => useDataPart,
  useDataParts: () => useDataParts,
...
function createChatStoreCreator(initialMessages = []) {
  ...
  return (set, get) => {
    ...
    return {
      ...
      _transientDataParts: /* @__PURE__ */ new Map(),
      // Chat helpers
      sendMessage: void 0,
      regenerate: void 0,
      stop: void 0,
      ...
      clearTransientDataParts: () => {
        ...
        batchUpdates(() => set({ _transientDataParts: /* @__PURE__ */ new Map() }));
      }
    };
  };
}
...
function useChat(options = {}) {
  ...
  const wrappedOnData = (0, import_react3.useCallback)(
    (dataPart) => {
      if (dataPart.type?.startsWith("data-")) {
        if (typeof store.getState === "function") {
          const storeState = store.getState();
          if (dataPart.data === null || dataPart.data === void 0) {
  ...
  (0, import_react3.useEffect)(() => {
    ...
    const storeMessages2 = currentStoreState.messages ?? [];
    const chatMessages = chatHelpers.messages ?? [];
    let nextMessages = chatMessages;
    if (storeMessages2.length > 0) {
      if (chatMessages.length === 0) {
        nextMessages = storeMessages2;
      } else if (chatMessages.length < storeMessages2.length) {
        nextMessages = mergeMessages(storeMessages2, chatMessages);
      }
    }
    const stateData = {
      id: chatHelpers.id,
  ...
  const storeMessages = (0, import_zustand2.useStore)(
    store,
...
// src/use-data-parts.ts
var import_react4 = require("react");
function useDataParts() {
  ...
  return (0, import_react4.useMemo)(() => {
    ...
    for (const dataPart of dataParts) {
      ...
      if (!existing || dataPart.timestamp && existing.timestamp && dataPart.timestamp > existing.timestamp) {
        byType[key] = dataPart;
      }
    }
    return {
...
function useDataPart(type, options) {
  const messages = useChatMessages();
  ...
  const result = (0, import_react4.useMemo)(() => {
    ...
    for (const dataPart of dataParts) {
      if (dataPart.type === fullType) {
        if (!latest || dataPart.timestamp && latest.timestamp && dataPart.timestamp > latest.timestamp) {
          latest = dataPart;
        }
      }
    }
...
function extractDataPartsFromMessages(messages) {
  const dataParts = [];
          ...
          if (typeof result === "object" && result && "parts" in result) {
            ...
            if (Array.isArray(parts)) {
              for (const nestedPart of parts) {
                const typedPart = nestedPart;
                if (typedPart.type?.startsWith("data-") && typedPart.data !== void 0) {
                  dataParts.push({
                    type: typedPart.type,
                    data: typedPart.data,
