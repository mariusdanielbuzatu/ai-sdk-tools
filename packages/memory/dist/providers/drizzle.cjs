...
var DrizzleProvider = class {
  ...
  async getWorkingMemory(params) {
    ...
    const result = await this.db.select().from(workingMemoryTable).where((0, import_drizzle_orm.eq)(workingMemoryTable.id, id)).limit(1);
  ...
  async updateWorkingMemory(params) {
    ...
    const existing = await this.db.select().from(workingMemoryTable).where((0, import_drizzle_orm.eq)(workingMemoryTable.id, id)).limit(1);
    if (existing.length > 0) {
      await this.db.update(workingMemoryTable).set({
        content: params.content,
        updatedAt: now
      }).where((0, import_drizzle_orm.eq)(workingMemoryTable.id, id));
    } else {
  ...
  async getMessages(params) {
    ...
    const whereConditions = [
      (0, import_drizzle_orm.eq)(messagesTable.chatId, params.chatId)
    ];
    if (params.userId) {
      whereConditions.push(
        (0, import_drizzle_orm.or)(
          (0, import_drizzle_orm.eq)(messagesTable.userId, params.userId),
          (0, import_drizzle_orm.eq)(messagesTable.userId, null)
        )
      );
    }
    const whereCondition = whereConditions.length === 1 ? whereConditions[0] : (0, import_drizzle_orm.and)(...whereConditions);
    const result = await this.db.select().from(messagesTable).where(whereCondition).orderBy(
      (0, import_drizzle_orm.desc)(messagesTable.timestamp),
      (0, import_drizzle_orm.desc)(
        import_drizzle_orm.sql`CASE WHEN ${messagesTable.role} = 'assistant' THEN 1 ELSE 0 END`
      )
  ...
  async saveChat(chat) {
    ...
    const existing = await this.db.select().from(chatsTable).where((0, import_drizzle_orm.eq)(chatsTable.chatId, chat.chatId)).limit(1);
    if (existing.length > 0) {
      ...
      }).where((0, import_drizzle_orm.eq)(chatsTable.chatId, chat.chatId));
    } else {
  ...
  async getChats(params) {
    ...
    const conditions = [];
    if (params.userId) {
      conditions.push((0, import_drizzle_orm.eq)(chatsTable.userId, params.userId));
    }
    if (params.search) {
      conditions.push((0, import_drizzle_orm.like)(chatsTable.title, `%${params.search}%`));
    }
    let query = this.db.select().from(chatsTable);
    if (conditions.length > 0) {
      const whereCondition = conditions.length === 1 ? conditions[0] : (0, import_drizzle_orm.and)(...conditions);
      query = query.where(whereCondition);
    }
    query = query.orderBy((0, import_drizzle_orm.desc)(chatsTable.updatedAt));
    if (params.limit) {
  ...
  async getChat(chatId) {
    ...
    const result = await this.db.select().from(chatsTable).where((0, import_drizzle_orm.eq)(chatsTable.chatId, chatId)).limit(1);
    if (!result.length) return null;
    const row = result[0];
    return {
      chatId: row.chatId,
  ...
  async updateChatTitle(chatId, title) {
    const { chatsTable } = this.config;
    if (!chatsTable) return;
    const existing = await this.db.select().from(chatsTable).where((0, import_drizzle_orm.eq)(chatsTable.chatId, chatId)).limit(1);
    if (existing.length > 0) {
      await this.db.update(chatsTable).set({
        title,
        updatedAt: /* @__PURE__ */ new Date()
      }).where((0, import_drizzle_orm.eq)(chatsTable.chatId, chatId));
    } else {
      await this.saveChat({
        chatId,
        title,
  ...
  async deleteChat(chatId) {
    ...
    if (messagesTable) {
      await this.db.delete(messagesTable).where((0, import_drizzle_orm.eq)(messagesTable.chatId, chatId));
    }
    if (chatsTable) {
      await this.db.delete(chatsTable).where((0, import_drizzle_orm.eq)(chatsTable.chatId, chatId));
    }
  }
